...
import { FileText, Receipt, NotebookPen, ClipboardList } from 'lucide-react'; // Importar nuevo icono

const invoicingTabs = [
  { id: 'boletas', label: 'Emitir Boleta', icon: Receipt, path: '/invoicing/boletas' },
  { id: 'resumen-diario', label: 'Resúmenes y Bajas', icon: ClipboardList, path: '/invoicing/resumen-diario' }, // <-- Nueva Pestaña
  { id: 'facturas', label: 'Emitir Factura', icon: FileText, path: '/invoicing/facturas', disabled: true },
  { id: 'notas-credito', label: 'Notas de Crédito', icon: NotebookPen, path: '/invoicing/notas-credito', disabled: false },
];
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
        <TabsList className="grid w-full md:w-auto grid-cols-2 lg:grid-cols-4 bg-surface border border-border p-1 rounded-xl shadow-lg">
        ...
        <div className="mt-6">
          <Card className="bg-card border-border shadow-2xl">
            <CardContent className="p-6">
              {/* The Outlet renders the specific page (e.g., BoletasPage, ResumenDiarioPage) */}
              <Outlet />
            </CardContent>
          </Card>
        </div>
